...
\noindent
Copyright \copyright 2011,2026 Ji{\v r}\'i Lebl
...
\noindent
See
\url{https://www.jirka.org/scv-mini/} or
\url{https://jirilebl.github.io/scv-mini/}
...
example, neither the Levi form nor pseudoconvexity is mentioned.  The focus is
...
fixed.
See \url{https://www.jirka.org/scv-mini/} or
\url{https://jirilebl.github.io/scv-mini/}
for more info.
...
$f \colon U \to \C$ be holomorphic.  Suppose that
...
We find $\Delta_\rho(a) \subset U$.  Expand $f$
...
\in \C \}$ for a $v \in \C^n \setminus \{ 0 \}$.
...
of a degree $k$ polynomial whose coefficients depend holomorphically on $z'$.
...
Furthermore, for each point $p \in U'$ there exists a neighborhood $N
\subset U'$ of $p$
...
of holomorphic functions by \propref{prop:rootshol}.  As before,
above each point of $E$ we only have finitely many points of $Z_f$,
and the conclusion
follows.
\end{proof}
...
that there does not exist a germ $(Y,p) \subset (X,p)$
...
\label{localparthm}
Let $(V,0)$ be an irreducible germ of a complex variety of dimension $k$
...
but by far not sufficient condition for having a continuous inverse.
...
We prove an immensely useful result for CR geometry.  As a consequence,
we can treat $z$ and $\bar{z}$ as separate
variables.  First we define
the complex conjugate
for a domain $U \subset \C^n$
\begin{equation}
...
We start with relating real polynomials and Hermitian forms.  We will
...
\begin{proof}
Take $v_1,\ldots,v_{a+b}$ to be an orthonormal set of eigenvectors of
...
\begin{proof}
We start with the easy direction.  Suppose that there
...
\begin{proof}
Without loss of generality, assume that $V$ is a neighborhood
of the origin where the power series for both $F$ and $G$ converges.
Denote by $F$ the $m \times \infty$ matrix of coefficients
of the function $F(z)$ and similarly with $G$.  
Let $\sZ$ be an infinite column vector of all the monomials in $z$
...
form $\{ z : z_3 = 0, z_1 = e^{i\theta} z_2 \}$ for some fixed $\theta$.
...
\end{equation}
We have added a higher order term only, yet this modification produced a
...
$G(z) = \sum g_\alpha {(z-p)}^\alpha$ (where $f_\alpha$ and $g_\alpha$ are in
$\ell^2$).
...
$X$ through $p$ is contained in a subvariety of
$\Delta_{\alpha}(p) \cap M$.  In fact,
\thmref{thm:holinm} applies in a small neighborhood of $p$.
That is, there is some $\beta > 0$ (smaller than $\alpha$) such that
for every
$q \in B(\beta;p) \cap M$
...
and
we notice that $\psi$ attains its maximum on $\tilde{X}$ away from the boundary
...
If $f \colon \bB_n \to \bB_n$ is a proper holomorphic map and $n \geq 2$, then $f$
...
Notice that $z \mapsto z^d$ is a proper map of the disc to the disc for any
...
$f \otimes g$ means
\begin{equation}
f \otimes g = \oplus_{j=1}^n \oplus_{k=1}^m f_j g_k
\end{equation}
...
We start with setting up a real-algebraic problem.  Let
$F \colon \bB_n \to \bB_N$ be a proper monomial map.  That is,
\begin{equation}
\norm{F(z)}^2 = 1 \quad \text{when} \quad  \norm{z}^2  = 1.
\end{equation}
If a component of $F$ is $z^\alpha$, then
...
then $F$ is spherically equivalent to one map
of the form
\begin{equation}
...
\end{smallmatrix}
\right]$
with $0 \leq t_1 \leq t_2 \leq \dots \leq t_n \leq 1$.
If $F(0) = 0$, then
...
for $\lambda > 0$.  We can then simply scale $f$ or $g$ and use the proposition.
...
\\
0 \leq t_1 \leq t_2 \leq \dots \leq t_n \leq 1.
\end{multline}
...
We end the course with a quick tour of the degree estimates proof for monomial maps.
...
if the corresponding monomials $m_1$ and $m_2$ satisfy
